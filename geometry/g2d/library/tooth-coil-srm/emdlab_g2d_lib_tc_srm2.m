function emdlab_g2d_lib_tc_srm2(g, ID, OD, Ntooth, g_beta, g_wy, name1, name2)
...
p6 = g.addPoint((ID/2+wy)*cos(alpha_t/2),(ID/2+wy)*sin(alpha_t/2));
p7 = g.addPoint((ID/2)*cos(alpha_t/2),(ID/2)*sin(alpha_t/2));
p8 = g.addPoint((ID/2+wy)*cos(gamma_tr/2),(ID/2+wy)*sin(gamma_tr/2));

% adding edges
e1 = g.addSegment(p2, p3);
e2 = g.addArc(p1, p3, p4, 1);
e3 = g.addSegment(p4, p8);
e4 = g.addArc(p1, p8, p6, 1);
e5 = g.addSegment(p6, p7);
e6 = g.addArc(p1, p7, p2, 0);
e7 = g.addArc(p1, p4, p5, 1);
e8 = g.addSegment(p5, p6);
