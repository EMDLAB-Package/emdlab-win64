...
% a 2-dimensional magnetic-static solver based on magnetic netwrok theory

classdef emdlab_solvers_ms2d_mn < handle & emdlab_ui_console
    ...
    methods
        ...
        function obj = emdlab_solvers_ms2d_mn(m)
        ...
        function assignElementData(obj)
            ...
            for i = 1:obj.m.Nmzs
                ...
                if obj.m.mts.(mzptr.material).MagneticPermeability.isTemperatureDependent 
                    ...
                    if obj.m.mts.(mzptr.material).MagneticPermeability.isIsotropic
                        obj.edata.MagneticPermeability(:,obj.m.ezi(:, mzptr.zi)) = obj.m.mts.(mzptr.material).MagneticPermeability.value;
                    else
...
%                         uij = uij / norm(uij);
%                         die = edgeCenter(abs(obj.m.elements(i,j)),:) - elementCenter(i,:);
%                         die = abs(die * obj.m.nEdges(abs(obj.m.elements(i,j)),:)');
%                     else
...
if obj.m.nbs(i,j)
    ...
    % direction from cell i to cell j
    d = Cj - Ci;
    uij = d / norm(d);

    % intersection of center-center line with face
    t = dot(F - Ci, n) / dot(d, n);
    P = Ci + t * d;

    % distance from cell center i to intersection point
    die = norm(P - Ci);
    ...
    uij = d / die;
        ...
        function evalTn(obj)
            obj.results.Tn = emdlab_mex_solvers_ts2d_evalTn(obj.results.T, obj.m.getCenterOfElements, obj.m.nodes, [obj.m.cl,size(obj.m.cl,2)*ones(obj.m.Ne,1)]);
        end
...
%             end          
        ...
        function varargout = plotMeshZoneAverageTemperature(obj, mzName, N, varargin)

            [f,ax] = obj.m.showwf;
            if nargin<3, N=10; end

            tmp = obj.m.cl(obj.m.ezi(:,obj.m.mzs.(mzName).zi),:);
            patch('faces', tmp, 'Vertices', obj.m.nodes, 'FaceVertexCData',obj.results.T(obj.m.ezi(:,obj.m.mzs.(mzName).zi)), ...
                'FaceColor','flat', 'edgecolor', 'none');
            colormap(jet(N));
            cb = colorbar;
            cb.FontName = 'Verdana';
        ...
        function varargout = plotMeshZoneTemperature(obj, mzName, N, varargin)
            ...
            cb.FontSize = 12;
            cb.Label.String = 'Temperature [C]';
            clim([min(obj.results.Tn(obj.m.mzs.(mzName).l2g)), max(obj.results.Tn(obj.m.mzs.(mzName).l2g))]);
            climits = clim; 
            cb.Ticks = fix(linspace(climits(1), climits(2), 10)*100)/100;
            cb.Ticks(1) = cb.Ticks(1) + 0.01;
        ...
        function y = getMeshZoneTemperature(obj, mzName)
            ...
            y = [min(obj.results.T(idx)), max(obj.results.T(idx)), obj.m.gea(idx)*obj.results.T(idx)/sum(obj.m.gea(idx))];
        end
