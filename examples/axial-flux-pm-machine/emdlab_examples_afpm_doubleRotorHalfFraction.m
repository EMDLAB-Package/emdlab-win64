...
emdlab_g2d_lib_tc110(g,gv_wst,gv_wss,gv_dss,5,2,25,0,0,0,'stator','sca','sap');
...
% generate full mesh
m.aux_cmyjcshj('rotorFront',gv_p/2,gv_taup,0)
m.aux_cmyjcshj('rapFront',gv_p/2,gv_taup,0)
m.aux_cmyjcsh('magnetFront',gv_p/2,gv_taup,0)
m.aux_cmyjcshj('stator',gv_Ns/2,gv_taus,0)
m.aux_cmycsh('sca',gv_Ns/2,gv_taus,0)
m.aux_cmyjcshj('sap',gv_Ns/2,gv_taus,0);

% add airgap mesh
m.aux_addLineAirGap('ag',0,gv_dss/2+gv_gap/2,1,0,gv_gap,2)
...
k2 = m.getfbniol_p0x([0,0]);
s.setAzBC(k1,0);
[km,ks] = m.splitShift(setdiff(m.getfbn,[k1;k2]), [gv_L/2,0]);
